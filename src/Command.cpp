...
// Ping
void PingCommand::Execute(const std::shared_ptr<Client> client, const std::vector<std::string> tokens){
    if (tokens.empty()) {
...
// Password
void PassCommand::Execute(std::shared_ptr<Client> client, const std::vector<std::string> tokens){
    if (client->IsAuth())
...
// Cap
void CapCommand::Execute(std::shared_ptr<Client> client, std::vector<std::string> tokens) {
    if (client)
        client->Write("CAP * LS :");
}

//Nick
void NickCommand::Execute(std::shared_ptr<Client> client, const std::vector<std::string> tokens){
    if(!client->IsAuth()){
      client->Reply(ERR_PASSWDMISMATCH(client->GetNickname()));
      server_->ClientDisconnect(client->GetFd());
...
// User
void UserCommand::Execute(std::shared_ptr<Client> client, const std::vector<std::string> tokens){
    if(client->GetNickname().empty()){
    	client->Reply(ERR_NONICKNAMEGIVEN(client->GetNickname()));
    	server_->ClientDisconnect(client->GetFd());
...
// Quit
void QuitCommand::Execute(std::shared_ptr<Client> client, const std::vector<std::string> tokens){
    std::string message;
    ...
    if (tokens[0].empty())
        message = " Leaving";
    if (client->GetChannel()) {
        const std::shared_ptr<Channel> aux = client->GetChannel();
...
// Join
void JoinCommand::Execute(std::shared_ptr<Client> client, std::vector<std::string> tokens){
    if (!client->IsAuth())
        return;
    if (tokens.empty()) {
    ...
    std::shared_ptr<Channel> aux = server_->FindChannel(name);
    if (!aux) {
...
// Msg
void MsgCommand::Execute(std::shared_ptr<Client> client, const std::vector<std::string> tokens) {
    if (tokens.size() < 2) {
    ...
    if (tokens[0].at(0) == '#') {
        std::shared_ptr<Channel> aux = server_->FindChannel(tokens[0]);
    ...
    std::shared_ptr<Client> dst = server_->FindClient(tokens[0]);
    if (dst == NULL) {
...
// Kick
void KickCommand::Execute(std::shared_ptr<Client> client, const std::vector<std::string> tokens) {
    if (tokens.size() < 2) {
        client->Reply(ERR_NEEDMOREPARAMS(client->GetNickname(), "KICK"));
        return ;
    }
    std::shared_ptr<Channel> channel = server_->FindChannel(tokens[0]);
    if (channel == NULL) {
        client->Reply(ERR_NOSUCHCHANNEL(client->GetNickname(), tokens[0]));
        return ;
    }
    ...
    std::shared_ptr<Client> dst = server_->FindClient(tokens[1]);
    if (dst == NULL) {
...
// Part
void PartCommand::Execute(std::shared_ptr<Client> client, std::vector<std::string> tokens) {
    if (tokens.empty()) {
        client->Reply(ERR_NEEDMOREPARAMS(client->GetNickname(), "PART"));
        return ;
    }
    const std::shared_ptr<Channel> channel = server_->FindChannel(tokens[0]);
    if (channel == NULL) {
        client->Reply(ERR_NOSUCHCHANNEL(client->GetNickname(), tokens[0]));
        return ;
    }
...
void ModeCommand::Execute(std::shared_ptr<Client> client, std::vector<std::string> tokens) {
    ...
    std::shared_ptr<Client> dst = nullptr;
...
//topic
void TopicCommand::Execute(std::shared_ptr<Client> client, const std::vector<std::string> tokens) {
    if (tokens.size() < 3) {
...
void InviteCommand::Execute(std::shared_ptr<Client> client, const std::vector<std::string> tokens) {
    if (tokens.size() != 2) {
        client->Reply(ERR_NEEDMOREPARAMS(client->GetNickname(), "TOPIC"));
        return ;
    }
